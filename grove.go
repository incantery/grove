// Package grove is the worktree-per-task model: one git worktree, one
// branch, one place to work in it, and a lifecycle that ends with the
// branch merged and all three gone. Git owns the worktrees — this is a
// thin layer over `git worktree` — and a Place owns the sessions; the
// package is what ties a checkout to where you work in it.
//
// Layout: a worktree for repo R named N lives beside the repo at
// <parent-of-R>/R--N, and its session is named R--N — unambiguous
// across repos, and it shows up in zoxide like any other directory.
// rook and vera's fleet both kept this convention before grove did, so
// their worktrees and grove's are the same worktrees.
package grove
...
type Repo struct {
	...
	// Place is where worktrees are worked in. Nil is None: git only.
	Place Place
}
...
type Worktree struct {
	...
	Live    bool   `json:"live"` // the session exists in the Place
}

// Conventions are what a fresh checkout needs that git does not carry.
...
// Merge is these conventions with more added, without duplicates.
func (c Conventions) Merge(more Conventions) Conventions {
	return Conventions{Copy: union(c.Copy, more.Copy), Link: union(c.Link, more.Link)}
}
...
// true home, so any checkout of the repo gets the same Repo. The
// Place is detected from the environment; set it after for another.
func Find(dir string) (Repo, error) {
	...
	return Repo{Root: root, Name: filepath.Base(root), Place: Detect()}, nil
}

func (r Repo) place() Place {
	if r.Place == nil {
		return None{}
	}
	return r.Place
...
// Session is the session a worktree is worked in: the directory's
// name, with the characters a multiplexer cannot carry in a name
// (tmux refuses `.` and `:`) turned to `_` — the rule rook has always
// used, so a session rook made and one grove names are the same.
func (r Repo) Session(name string) string {
	if name == "" {
		return SessionName(r.Root)
	}
	return SessionName(r.Path(name))
}

// SessionName is the session for a directory: its base name, scrubbed.
...
// dirtiness, distance from the default branch, and whether a session
// is open on it.
func (r Repo) List() ([]Worktree, error) {
	...
	for sc.Scan() {
		...
		case strings.HasPrefix(line, "worktree "):
			...
			cur = &Worktree{Path: p, Name: r.nameOf(p), Session: SessionName(p)}
		case cur == nil:
...
// Get finds the worktree named name, by short name, branch, or
// directory name.
func (r Repo) Get(name string) (Worktree, error) {
	...
	return Worktree{}, fmt.Errorf("no worktree %q (grove ls)", name)
}

// Main is the repo's own checkout as a row.
func (r Repo) Main() (Worktree, error) {
	wts, err := r.List()
	if err != nil {
		return Worktree{}, err
	}
...
// Fetch brings origin up to date, so that New can find a branch that
// exists only there. Kept separate from New because it needs the
// network and New does not: a caller that knows it is offline, or
// just fetched, skips it.
func (r Repo) Fetch() error {
	if out, err := git(r.Root, "fetch", "--quiet", "origin"); err != nil {
		return fmt.Errorf("git fetch: %s", strings.TrimSpace(out))
	}
	return nil
}

// New creates a worktree named name on a branch of the same name, in
// three tiers, stopping at the first that answers: the local branch
// if there is one; the branch on origin, tracked, if origin has it;
// else a fresh branch off `from` (the default branch when empty). Then
...
func (r Repo) New(name, from string, conv Conventions) (Worktree, error) {
	...
	switch {
	case r.hasRef("refs/heads/" + name):
		args = append(args, path, name)
	case r.hasRef("refs/remotes/origin/" + name):
		args = append(args, "--track", "-b", name, path, "origin/"+name)
	default:
		if from == "" {
...
func (r Repo) apply(path string, conv Conventions) {
	for _, rel := range conv.Copy {
		...
		if _, err := os.Lstat(src); err != nil {
			continue // nothing to copy; not an error
		}
		if err := copyPath(src, dst); err != nil {
			fmt.Fprintf(os.Stderr, "grove: copy %s: %v\n", rel, err)
		}
	}
...
func (r Repo) hasRef(ref string) bool {
	_, err := git(r.Root, "rev-parse", "--verify", "--quiet", ref)
	return err == nil
}

// Open makes sure a session exists for the worktree and goes there,
// the way the Place does it.
func (r Repo) Open(wt Worktree) error {
	if err := r.place().Open(wt.Session, wt.Path); err != nil {
		return fmt.Errorf("opening %s: %w", wt.Session, err)
	}
	return nil
}
...
// Remove deletes a worktree, ends its session, and deletes its branch.
// Without force it refuses dirty checkouts and unmerged branches.
// Every refusal happens before anything is touched: a worktree that is
// half-removed (tree gone, branch kept, session dead) is worse than
...
func (r Repo) Remove(wt Worktree, force bool) error {
	...
	if err := r.place().Close(wt.Session); err != nil {
		return fmt.Errorf("closing %s: %w", wt.Session, err)
	}
